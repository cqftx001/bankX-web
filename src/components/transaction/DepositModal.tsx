...
export function DepositModal({ open, onClose, onSuccess, accounts }: Props){
    const { t } = useTranslation();
    const [form] = Form.useForm<FormValues>();
    const [submitting, setSubmitting] = useState(false);
    const [submitError, setSubmitError] = useState<string | null>(null);

    // 整个 Modal 生命周期复用这个 key，
    // 用 useState lazy initializer 避免每次渲染都生成新 UUID。
    const [idempotencyKey, setIdempotencyKey] = useState(() => generateIdempotencyKey());

    // form实例一般不发生变化 一般不参与effect
    useEffect(() => {
        if (open) {
    ...
    return (
        <Modal
            title={t('transaction.modal.depositTitle')}
            open={open}
            onOk={handleSubmit}
            onCancel={onClose}
